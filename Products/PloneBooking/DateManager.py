...
class DateManager:
    ...
    def excludeFromBooking(self, day):
        from Products.PloneBooking.holidays import Holidays
        dday = datetime.datetime.fromtimestamp(day)
        holiday_list = [x[0] for x in Holidays(2015, None).get_holiday_list()]
        exclude = False
        if dday.weekday() in (5, 6):
            exclude = True
        if dday.date() in holiday_list:
            exclude = True
        return exclude
    ...
    def buildWeekTable(self, week, year, starting_hour=8, starting_minute=0, ending_hour=19, ending_minute=0, delta=3600):
        ...
        for hour in hours:
            week_table.append([])
        for day in days:
            # if self.excludeFromBooking(day):
            #     continue
